...
const Post: React.FC<BlogProps> = ({ blogData, currentUser }) => {
  ...
  useEffect(() => {
    const userLikedPosts = blogData
      .filter(blog => blog.likes && Array.isArray(blog.likes) && blog.likes.some(like => like.userId === currentUser?.id))
      .map(blog => blog.id);
  ...
  useEffect(() => {
    // Update blog state whenever blogData changes
    setBlog(blogData);
  }, [blogData]);
  ...
  const sortedBlogData = [...blog].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
